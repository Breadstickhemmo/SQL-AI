...
VECTORIZER_PATH = "models/decision_tree/dt_vectorizer_best.joblib"
MODEL_PATH = "models/decision_tree/decision_tree_model_best.joblib"
...
def load_vectorizer_and_model(vectorizer_path, model_path):
    ...
    print(f"Загрузка модели Decision Tree из: {model_path}")
    try:
    ...
    print("Векторизатор и модель Decision Tree успешно загружены.")
    return vectorizer, model

if __name__ == "__main__":
    start_total_time = time.time()
    vectorizer, model = load_vectorizer_and_model(VECTORIZER_PATH, MODEL_PATH)
    print(f"\n--- Автоматическое тестирование модели Decision Tree ---")
    ...
    if not true_labels:
        ...
        print("\n--- Результаты Тестирования (Decision Tree) ---")
        print(f"Общая точность (Accuracy): {accuracy:.4f}")
        print("\nОтчет по классам:")
        print(report)
        print("\n--- Неверно классифицированные запросы (Decision Tree) ---")
        errors_found = False
